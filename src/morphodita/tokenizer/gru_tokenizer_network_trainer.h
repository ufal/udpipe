      ...
      if (batch_size == 1 ||
        ...
        if (batch_size == 1)
          for (auto&& chosen_embedding : chosen_embeddings)
            chosen_embedding->update_weights(adam, lr);
        else
          for (auto&& embedding : embeddings)
            embedding.second.update_weights(adam, lr);
        gru_fwd.update_weights(adam, lr);
        gru_bwd.update_weights(adam, lr);
        projection_fwd.update_weights(adam, lr);
        projection_bwd.update_weights(adam, lr);
      }
    }
...
void gru_tokenizer_network_trainer<D>::matrix_trainer<R, C>::update_weights(bool adam, float learning_rate) {
  if (adam) {
    for (int i = 0; i < R; i++) {
      for (int j = 0; j < C; j++) {
...
template <int D>
void gru_tokenizer_network_trainer<D>::gru_trainer::update_weights(bool adam, float learning_rate) {
  X.update_weights(adam, learning_rate);
  X_r.update_weights(adam, learning_rate);
  X_z.update_weights(adam, learning_rate);
  H.update_weights(adam, learning_rate);
  H_r.update_weights(adam, learning_rate);
  H_z.update_weights(adam, learning_rate);
}
